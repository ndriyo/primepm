...
The visual reference for this feature lives in the **PrimePM Design System** Figma file, on a dedicated page:

- **Mockup page** — `Spec 002 • Baseline Overlay` — <https://www.figma.com/file/N9DB0nKpW1qZiOj1AIHAlk?node-id=62-2>
- **Full mockup frame (all scenes)** — <https://www.figma.com/file/N9DB0nKpW1qZiOj1AIHAlk?node-id=63-2>

Per-scene deep-links (use these when implementing the matching component):

| Scene                                            | Implements                                                                       | Link                                                                                  |
| ------------------------------------------------ | -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| A. Tracking Gantt — overlay active (v1)          | `GanttChart`, `BaselineBar`, `TaskBar` variance treatments (FR-008 → FR-011)     | <https://www.figma.com/file/N9DB0nKpW1qZiOj1AIHAlk?node-id=66-2>                      |
| B. `Set baseline` dialog                         | `SetBaselineDialog` (FR-002, mandatory rationale, audit-log notice)              | <https://www.figma.com/file/N9DB0nKpW1qZiOj1AIHAlk?node-id=67-2>                      |
| C. Baseline-version selector (open)              | `BaselineVersionSelector` (FR-013, `'latest'` sentinel + per-version metadata)   | <https://www.figma.com/file/N9DB0nKpW1qZiOj1AIHAlk?node-id=67-28>                     |
| D. No baseline yet — selector hidden             | Byte-identical Gantt fallback (FR-014, SC-007)                                   | <https://www.figma.com/file/N9DB0nKpW1qZiOj1AIHAlk?node-id=68-2>                      |
| E. Baseline history                              | Audit-log surface (US5) + immutability cue (FR-005)                              | <https://www.figma.com/file/N9DB0nKpW1qZiOj1AIHAlk?node-id=68-39>                     |

The mockup also ships an `Overlay legend` block keyed by overlay state
(`unchanged` / `variant` / `added` / `removed`), so the contract names in
[`contracts/overlay-ui.contract.md`](./contracts/overlay-ui.contract.md)
map 1:1 to the visual treatments. Colors and typography come from
`src/index.css` (`--color-bar`, `--color-bar-progress`, `--color-bar-critical`)
and `src/styles/pp-tokens.css` — no new tokens introduced.
