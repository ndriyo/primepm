...
- Q: Should actuals edits be audited (versioned change history) in this release? → A: Yes — append-only audit log per edit capturing user, timestamp, and before/after values for % complete, actual start, and actual finish. (FR-016 added; Assumptions tightened to remove the "desirable but not strictly required" wording.)
- Q: When a PM saves % complete = 100 but leaves actual finish empty (or the converse), how should the system respond? → A: Auto-fill the missing field — if % complete = 100 is saved with no actual finish, set actual finish to today's date; if an actual finish is saved with % complete < 100, set % complete to 100. (FR-017 added; Edge Cases updated.)
- Q: Who should be able to view actuals (% complete, actual start, actual finish) on a task? → A: Anyone with read access to the project — same scope as baseline and current schedule (002 parity). Edit remains gated on task-edit permission. (FR-018 added; Assumptions tightened.)
- Q: What server-side latency target should the actuals-save endpoint meet? → A: p95 ≤ 500 ms at projects of ≤500 tasks, matching the 002 baseline-save target. (SC-010 added.)
- Q: When a task is deleted, what should happen to its actuals audit log entries? → A: Retain all prior entries and emit a final `task-deleted` audit event (actor, timestamp); audit entries are never cascade-deleted with the task. (FR-019 added; FR-012 scoped to actuals records only.)
- Q: FR-001 names percent complete as one of three actuals fields, but the 002 `BaselineTask` contract has `progressPct` as a required field of every baseline snapshot — does that violate FR-013 ("actuals MUST NOT be in any baseline")? → A: No. The **baseline is conceptually the planned schedule** — planned start, finish, duration, effort, cost. The 002 snapshot persists the planned schedule plus convenience-copies of other task fields (name, notes, color, progress at capture time) so the snapshot is self-contained for rendering. `progressPct` is therefore a **convenience-copy of the latest value at capture time**, not "actuals data" within the baseline. The new actuals date fields `actualStart` and `actualFinish` introduced by this feature, however, MUST NOT appear in the baseline snapshot in any form. (FR-013 wording tightened; plan.md "Constraints" updated to remove the misleading "loadSnapshot captures only the baseline-relevant columns" claim.)
...
- A task is saved with % complete = 100 but no actual finish recorded. The system auto-fills the actual finish with today's date as part of the same save (per FR-017). Likewise, if an actual finish is saved while % complete is below 100, the system auto-bumps % complete to 100. The auto-fill is reflected in the saved record and in the audit log entry's "after" values.
...
- A task has actuals recorded, then the PM deletes the task from the current schedule. The live actuals values are deleted alongside the task (no orphan actuals row), but the actuals audit log entries are retained per FR-019 — the system additionally emits a final `task-deleted` audit entry on the way out so the deletion is itself recorded for anti-gaming review. Audit retention is independent of any baseline behavior; actuals are never baselined.
...
- **FR-012**: Deleting a task MUST also delete its associated actuals records (the live percent-complete, actual-start, and actual-finish values on the task row). No orphan actuals records may remain. This cascade does NOT apply to the actuals audit log; audit entries are governed by FR-019.
- **FR-013**: The baseline of a task is conceptually its **planned schedule** — planned start, planned finish, planned duration, planned effort, and planned cost. The new actuals date fields introduced by this feature (`actualStart`, `actualFinish`) MUST NOT appear in any baseline snapshot in any form. The 002 snapshot may persist convenience-copies of other task fields (name, color, `progressPct` at capture time) so that the snapshot is self-contained for rendering; those copies are not "the baseline" and are not used by the variance overlay (which compares only planned-schedule fields). See `Clarifications → Session 2026-05-10` for the rationale.
...
- **FR-016**: Every create or update of a task's actuals MUST produce an append-only audit log entry recording: the task identifier, the acting user's identity, the timestamp of the change, and both the before and after values of all three actuals fields (percent complete, actual start, actual finish). Audit log entries MUST NOT be editable or deletable through any user-facing path. The audit trail for a task MUST be retrievable in chronological order on request.
- **FR-017**: When the user saves actuals such that % complete = 100 with an empty actual finish, the system MUST auto-fill the actual finish with today's calendar date as part of the same save. When the user saves actuals with a non-empty actual finish but % complete strictly less than 100, the system MUST auto-bump % complete to 100. The auto-filled value MUST be the value that is persisted, the value shown in the inspector after save, and the "after" value recorded in the audit log entry per FR-016. Auto-fill MUST NOT override an explicit user-entered value: if the user enters both fields explicitly, the entered values are saved as-is (subject to FR-006's actual-start/actual-finish ordering check).
- **FR-018**: Read access to a task's actuals (% complete, actual start, actual finish, derived summary rollups, and audit log) MUST follow the same access rule as baseline and current-schedule data: any user with read access to the project sees actuals. Write access to actuals remains gated on task-edit permission per FR-001. No separate actuals-only viewer or editor role is introduced in this release.
- **FR-019**: Actuals audit log entries MUST be retained when their task is deleted; they MUST NOT be cascade-deleted with the task. The task identifier on each retained entry remains as a frozen historical reference. On task deletion, the system MUST emit one final audit log entry of kind `task-deleted` recording the actor, the timestamp, the task identifier, and the last-known actuals values as the "before" snapshot (with no "after" snapshot). The audit history for a deleted task MUST remain retrievable in chronological order by task identifier for anti-gaming review.
...
- **Actuals audit entry**: An append-only record of a single actuals event — create, update, or `task-deleted` — on a task. Holds the event kind, the task identifier, the acting user, the change timestamp, and (for create/update) the before/after values of percent complete, actual start, and actual finish; the `task-deleted` kind records the last-known actuals values as the "before" snapshot with no "after" snapshot. Immutable; cannot be edited or deleted through user paths. Entries are retained when the task is deleted and remain retrievable by task identifier for anti-gaming review.
...
- **SC-010**: The server-side actuals-save round trip for a single task on a project of ≤500 tasks completes within 500 ms at the 95th percentile under realistic load (matches the 002 baseline-save latency envelope).
...
- "Permission to view actuals" inherits from "permission to read the project" — the same scope used today for baseline and current-schedule data (002 parity). No actuals-only viewer role exists in this release.
...
- Actuals are not versioned in the live data record (the current values are stored once per task and are overwritten on edit), but every edit MUST produce an append-only audit log entry per FR-016 capturing user, timestamp, and before/after values. The audit log is the change history; the task row holds only the latest value.
